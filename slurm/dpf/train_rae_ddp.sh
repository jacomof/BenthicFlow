...
REQUIRED=$(du -sbc "$REEF_SCRATCH_ROOT/depth-dpf" "$REEF_SCRATCH_ROOT/features-dpf" | tail -1 | awk '{print $1}')
...
# memory.py is for one-time .npz → .npy conversion; features are already in .npy format
# python -u scripts/convert_features_npz_to_npy.py

# Launch training
torchrun --standalone --nproc_per_node=4 scripts/train_rae_ddp_dpf.py --batch 16 --amp