...
def sample_conditioning(campaign, rng, device, deployment=None,
...
"""
    ...
    for attempt in range(1, max_tries + 1):
        ...
        print(f"  [accept]    {campaign}/{dep}[{idx}] "
    ...
    raise RuntimeError(
        ...
        + (f"; cosine>={sim_thr} to anchor" if anchor is not None and sim_thr > 0 else "")
        + "). Lower --sim-thr or pin an easier deployment.")


@torch.no_grad()
def generate_interp_panorama(model, rae, cond_left, cond_right, n_windows=4,
                             n_steps=100, cfg_scale=3.0, temperature=1.0,
                             device="cuda"):
...
def embed_windows(dino_model, crops):
    ...
    crops = F.interpolate(crops, size=(DINO_NATIVE, DINO_NATIVE),
...
def compute_embeddings(args, device):
    ...
    for k in range(args.n_panoramas):
        ...
        seed_l_rgb, cond_l, key_l = sample_conditioning(
            LEFT_CAMPAIGN, rng, device, deployment=args.left_deployment,
            anchor=anchor_l, sim_thr=args.sim_thr)
        seed_r_rgb, cond_r, key_r = sample_conditioning(
            RIGHT_CAMPAIGN, rng, device, deployment=args.right_deployment,
            anchor=anchor_r, sim_thr=args.sim_thr)
        if anchor_l is None:
            anchor_l, anchor_r = cond_l, cond_r
        keys_l.append(key_l); keys_r.append(key_r)

        pano_rgb, px_starts, win_alphas = generate_interp_panorama(
            model, rae, cond_l, cond_r, n_windows=args.n_windows,
            n_steps=args.n_steps, cfg_scale=args.cfg_scale, device=device)

        crops = torch.cat([pano_rgb[:, :, :, p:p + 224] for p in px_starts])  # [n_w,3,224,224]
        ...
        panos.append((pano_rgb[0].permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8))
    ...
    np.savez_compressed(
        CACHE_NPZ, embeddings=emb, alphas=alphas,
        panos=np.stack(panos), seeds_l=np.stack(seeds_l), seeds_r=np.stack(seeds_r),
        seed_keys_l=np.array(keys_l), seed_keys_r=np.array(keys_r),
        left_campaign=LEFT_CAMPAIGN, right_campaign=RIGHT_CAMPAIGN, seed=args.seed)
...
def demo_embeddings(args):
    ...
    for k in range(n):
        for i, al in enumerate(alphas):
            ...
            emb[k, i] = ((1 - al) * a + al * b + 0.8 * al * (1 - al) * curve
...
def reduce_2d(emb, seed, perplexity=None):
    ...
    xy = TSNE(n_components=2, perplexity=perplexity, init="pca",
              learning_rate="auto", random_state=seed).fit_transform(flat)
...
def plot_contact_sheet(seeds_l, panos, seeds_r, out_path):
    ...
    fig, axes = plt.subplots(n, 3, figsize=(7.0, row_h * n * 1.06), squeeze=False,
    ...
    for k in range(n):
        for ax, im in zip(axes[k], (seeds_l[k], panos[k], seeds_r[k])):
            ax.imshow(im)
            ax.set_axis_off()
        axes[k, 1].text(0.01, 0.95, f"#{k}", transform=axes[k, 1].transAxes,
                        ha="left", va="top", fontsize=6, color="white",
                        bbox=dict(facecolor="black", alpha=0.5, pad=1,
                                  edgecolor="none"))
    for ax, title in zip(axes[0], (f"Seed L\n{LEFT_CAMPAIGN}",
...
def plot_tsne(xy, out_stem, images=None, example=-1):
    ...
    left_site = name_mappings.get(LEFT_CAMPAIGN.rstrip("0123456789"), LEFT_CAMPAIGN.rstrip("0123456789"))
    right_site = name_mappings.get(RIGHT_CAMPAIGN.rstrip("0123456789"), RIGHT_CAMPAIGN.rstrip("0123456789"))
    labels = ([f"{left_site} seed"]
    ...
    if images is None:
        ...
        gs = fig.add_gridspec(2, 3, height_ratios=[2.75, strip_h],
                              width_ratios=[1, n_w, 1], hspace=0.16, wspace=0.05)
        ax = fig.add_subplot(gs[0, :])
        ax_strip = [fig.add_subplot(gs[1, j]) for j in range(3)]

    for k in range(n):                                   # trajectories underneath
        ax.plot(xy[k, :, 0], xy[k, :, 1], color="0.55", lw=0.5, alpha=0.35,
    ...
    for i in range(n_items):
        ...
        ax.scatter(xy[:, i, 0], xy[:, i, 1],
                   ...
                   facecolor=colors[i], edgecolor="0.25", linewidth=0.35,
                   label=labels[i], zorder=3 if is_seed else 2)
    ...
    legend_kw = dict(ncol=n_items, frameon=False, handletextpad=0.15,
                     columnspacing=0.8, borderaxespad=0.0)
    if images is None:
        ax.legend([handles[j] for j in order], [lbls[j] for j in order],
                  loc="upper center", bbox_to_anchor=(0.5, -0.09), **legend_kw)
    else:
        ax.legend([handles[j] for j in order], [lbls[j] for j in order],
                  loc="lower center", bbox_to_anchor=(0.5, 1.02), **legend_kw)
    ...
    if images is not None:
        ...
        k = example if 0 <= example < n else int(
            np.argmin(((xy - cent) ** 2).sum(-1).sum(-1)))
        ...
        for i in range(n_items):
            is_seed = i in (0, n_items - 1)
            ax.scatter(*xy[k, i], s=34 if is_seed else 24,
                       marker="D" if is_seed else "o", facecolor=colors[i],
                       edgecolor="black", linewidth=0.8, zorder=5)

        for a, im, lab in zip(ax_strip, (seeds_l[k], panos[k], seeds_r[k]),
                              (f"{left_site} seed", "Generated panorama",
                               f"{right_site} seed")):
            a.imshow(im)
            a.set_xticks([]); a.set_yticks([])
            for s in a.spines.values():
                s.set_linewidth(0.5); s.set_color("0.5")
        ...
        anchors = ([(ax_strip[0], (seeds_l.shape[2] / 2, 0), 0)]
                   + [(ax_strip[1], ((i + 0.5) * w_px, 0), i + 1)
                      for i in range(n_w)]
                   + [(ax_strip[2], (seeds_r.shape[2] / 2, 0), n_items - 1)])
        for axA, xyA, i in anchors:
            fig.add_artist(ConnectionPatch(
                xyA=xyA, coordsA="data", axesA=axA,
                xyB=tuple(xy[k, i]), coordsB="data", axesB=ax,
                arrowstyle="-|>", mutation_scale=5, lw=0.55,
                color="0.3", alpha=0.85, shrinkB=2.5))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--left-deployment", default=LEFT_DEPLOYMENT,
                    help="pin the left seed to one deployment (None = random per panorama)")
    ap.add_argument("--right-deployment", default=RIGHT_DEPLOYMENT,
    ...
    if args.list_deployments:
        for c in (LEFT_CAMPAIGN, RIGHT_CAMPAIGN):
            ...
            n_imgs = {d: len(np.load(RGB_SRC / c / f"{d}.npy", mmap_mode="r"))
                      for d in deps}
    ...
    if args.demo:
        ...
        if args.plot_only:
            if not CACHE_NPZ.exists():
                sys.exit(f"--plot-only but no cache at {CACHE_NPZ}; run without it first.")
        else:
