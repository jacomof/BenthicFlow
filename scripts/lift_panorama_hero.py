...
from scripts.test_panorama import FEAT_SRC, RGB_SRC, sample_conditioning
...
def load_seed(
    ...
    if not rgb_npy.exists():
        ...
        sys.exit(
            f"Seed {campaign}/{deployment}: no such deployment. Available:\n  "
            + "\n  ".join(avail)
        )
    ...
    for attempt in range(1, max_tries + 1):
        ...
        if not 0 <= i < len(rgb_arr):
            sys.exit(
                f"Seed {campaign}/{deployment}[{i}]: index out of range "
    ...
    raise RuntimeError(
        f"No well-exposed frame found in {campaign}/{deployment} "
    ...
    for i, tl in enumerate(tilts):
        for j, az in enumerate(azims):
            ...
            if save_individual:
                ...
                plt.imsave(
                    angle_dir / f"beauty_t{tl:02d}_a{az:+03d}.png", np.clip(rgba, 0, 1)
                )
    fig.suptitle(
        "Hero-angle contact sheet (or frame surfels.ply in SuperSplat)", fontsize=12
    )
    fig.tight_layout()
...
def main():
    ...
    out_dir = args.out or (
        FIG_ROOT
        / CFM_RUN_NAME
        / f"hero_4_{args.cond_jitter:.2f}_{args.experiment_label}"
    )
    ...
    if args.load_rgbd and args.load_rgbd.exists():
        ...
        print(
            f"Generating {args.n}x{args.n} panorama "
    ...
    print(
        f"  provenance saved: {', '.join(f'{c}={m}' for c, m in zip(CORNERS, seed_ids))}"
    )
    ...
    save_angle_sheet(
        ...
        out_dir,
        device,
        save_individual=args.save_all_angles,
    )
    ...
    save_heatmap(
        alpha_metric,
        out_dir / "coverage_map.png",
        "front-view alpha (coverage)",
        cmap="gray",
        vmax=1.0,
    )
    ...
    print(
        "Headline candidates: provenance.png (corners-in-context), "
