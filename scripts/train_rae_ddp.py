...
print(f"Training RAE on device {DEVICE} with native size {NATIVE_SIZE}, train size {TRAIN_SIZE}, patch size {PATCH_SIZE}, native grid {NATIVE_GRID}, train grid {TRAIN_GRID}")
...
class CSVSplitDataset(Dataset):
        ...
        with csv_path.open("r", newline="") as handle:
            ...
            for row in reader:
                ...
                if campaigns is not None and campaign not in campaigns:
                    print(f"Skipping row with campaign '{campaign}' not in filter list.")
                    continue
                ...
                if not depth_npy.exists() or not keys_npy.exists() or not dino_npy.exists():
                    print(f"Skipping row with missing files for campaign '{campaign}', deployment '{deployment}': {rel_path}")
                    continue
                ...
                # Extract the exact filename from the path to match the keys array
                filename = Path(rel_path).name 
                depth_keys = depth_key_cache[dep_key]
                
                if filename in depth_keys:
                    self.records.append((campaign, deployment, rel_path, depth_keys[filename]))

        if not self.records:
            raise RuntimeError(f"CSVSplitDataset is empty for {csv_path}. Check paths and extractions.")
    ...
    def __getitem__(self, i: int):
        ...
        # Load targets from MMap
        depth = torch.from_numpy(np.asarray(depth_arr[idx], dtype=np.float32)).unsqueeze(0)
        ...
        if self.train:
            ...
            y_patch, x_patch = (NATIVE_GRID - TRAIN_GRID) // 2, (NATIVE_GRID - TRAIN_GRID) // 2
            y0, x0 = y_patch * PATCH_SIZE, x_patch * PATCH_SIZE
...
def save_recon_grid(rae: RAE, batch, out_path: Path, n: int = 8):
    ...
    depth_real_3 = depth.repeat(1, 3, 1, 1)
    depth_pred_3 = depth_pred.repeat(1, 3, 1, 1)
    grid = torch.cat([rgb, rgb_pred, depth_real_3, depth_pred_3], dim=0)
    img = make_grid(grid, nrow=n, padding=2)
...
@torch.no_grad()
def run_evaluation(rae: RAE, loss_fn: RAELoss, dataloader, 
                   max_batches: int | None = None) -> dict:
    rae.eval()
    token_norm = TokenLayerNorm().to(DEVICE)
    
    sums = {"l1_rgb": 0.0, "l1_depth": 0.0, "lpips": 0.0,
            "fused_mean": 0.0, "fused_std": 0.0, "n": 0}
...
def build_models() -> tuple[RAE, DinoDiscriminator, RAELoss, TokenLayerNorm]:
    ...
    loss_fn = RAELoss(
        lpips_start_epoch=6, gan_start_epoch=8,
        w_l1_rgb=1.0, w_depth=1.0, w_lpips=1.0, w_gan=0.75,
        depth_loss="l1", w_depth_grad=0.0,   # base variant: [0,1] relative depth
    ).to(DEVICE)
    token_norm = TokenLayerNorm().to(DEVICE)
    return rae, disc, loss_fn, token_norm


def build_optimizers(rae: nn.Module, disc: nn.Module, lr: float, total_epochs: int, steps_per_epoch: int):
    ...
    def make_sched(opt):
        warm = LinearLR(opt, start_factor=0.01, end_factor=1.0, total_iters=warmup_steps)
...
def save_checkpoint(ckpt_dir: Path, epoch: int, global_step: int,
                    rae: nn.Module, disc: nn.Module,
                    g_opt: torch.optim.Optimizer, d_opt: torch.optim.Optimizer,
                    g_sched, d_sched, ema: EMA, args: argparse.Namespace):
    checkpoint = {
        "epoch": epoch + 1,
...
def load_checkpoint(resume_path: str | Path,
                    rae: nn.Module, disc: nn.Module,
                    g_opt: torch.optim.Optimizer, d_opt: torch.optim.Optimizer,
                    g_sched, d_sched, ema: EMA) -> tuple[int, int]:
...
def main():
    ...
    if is_ddp:
        ...
        train_dl = DataLoader(train_ds, batch_size=args.batch, sampler=train_sampler,
    ...
    if global_rank == 0:
        ...
        test_dl = DataLoader(test_ds, batch_size=args.batch, shuffle=False, num_workers=4, pin_memory=True)
    ...
    g_opt, d_opt, g_sched, d_sched = build_optimizers(rae, disc, args.lr, args.epochs, steps_per_epoch)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        pbar = tqdm(train_dl, desc=f"epoch {epoch}/{args.epochs}", dynamic_ncols=True) if global_rank == 0 else train_dl
        running = {}
        ...
        for rgb, depth, rgb_latent in pbar:
            ...
            if amp_dtype is not None:
                with torch.autocast(device_type="cuda", dtype=amp_dtype):
                    ...
                    g_out = loss_fn.generator_loss(
                        rgbd_pred=rgbd_pred, rgbd_target=rgbd_target,
            ...
            if loss_fn.gan_active:
                ...
                with torch.no_grad():
                    if amp_dtype is not None:
                        with torch.autocast(device_type="cuda", dtype=amp_dtype):
                            rgbd_fake, _ = get_model(rae)(rgb_latent, depth)
                    else:
                        rgbd_fake, _ = get_model(rae)(rgb_latent, depth)
                ...
                if amp_dtype is not None:
                    with torch.autocast(device_type="cuda", dtype=amp_dtype):
                        d_out = loss_fn.discriminator_loss(disc=disc, rgb_real=rgb, rgb_fake=rgb_fake)
                else:
                    d_out = loss_fn.discriminator_loss(disc=disc, rgb_real=rgb, rgb_fake=rgb_fake)

                d_loss = d_out["d_total"]
                d_loss.backward()
                torch.nn.utils.clip_grad_norm_([p for p in get_model(disc).head.parameters()], max_norm=1.0)
                d_opt.step()
                d_sched.step()
            else:
            ...
            if global_rank == 0:
                for k, v in g_out.items():
                    if k != "total" and torch.is_tensor(v): running.setdefault(k, []).append(float(v))
                for k, v in d_out.items():
                    if torch.is_tensor(v): running.setdefault(k, []).append(float(v))

                if global_step % 50 == 0:
                    msg = " ".join(f"{k}={sum(vs[-50:])/min(50,len(vs)):.3f}" for k, vs in running.items())
        ...
        if global_rank == 0:
            if (epoch + 1) % args.val_every == 0 or epoch == args.epochs - 1:
                ...
                val_metrics = run_evaluation(get_model(rae), loss_fn, val_dl, max_batches=16)
                save_recon_grid(get_model(rae), val_batch_for_figs, fig_dir / f"recon_e{epoch:03d}.png")
                ...
                with open(ckpt_dir / "metrics.jsonl", "a") as f:
                    f.write(json.dumps({"epoch": epoch, "step": global_step, **val_metrics}) + "\n")
    ...
    if global_rank == 0:
        ...
        backup = ema.apply_to(get_model(rae))
        test_metrics = run_evaluation(get_model(rae), loss_fn, test_dl, max_batches=None)
