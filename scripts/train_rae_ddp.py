...
from models.losses import DinoDiscriminator, RAELoss
...
class CSVSplitDataset(Dataset):
        ...
        with csv_path.open("r", newline="") as handle:
            ...
            for row in reader:
                ...
                if (
                    ...
                    print(
                        f"Skipping row with missing files for campaign '{campaign}', deployment '{deployment}': {rel_path}"
                    )
                    continue
    ...
    def __getitem__(self, i: int):
        ...
        depth = torch.from_numpy(
            np.asarray(depth_arr[idx], dtype=np.float32)
        ).unsqueeze(0)
...
def build_models() -> tuple[RAE, DinoDiscriminator, RAELoss, TokenLayerNorm]:
    ...
    loss_fn = RAELoss(
        ...
        w_gan=0.75,
        depth_loss="l1",
        w_depth_grad=0.0,  # base variant: [0,1] relative depth
    ).to(DEVICE)
    token_norm = TokenLayerNorm().to(DEVICE)
        ...
        for rgb, depth, rgb_latent in pbar:
            ...
            if loss_fn.gan_active:
                ...
                torch.nn.utils.clip_grad_norm_(
                    [p for p in get_model(disc).head.parameters()], max_norm=1.0
                )
                d_opt.step()
                d_sched.step()
            else:
