...
from scripts.test_panorama import FEAT_SRC, RGB_SRC
...
def scan_campaign(
    ...
    for dep in deps:
        ...
        for i in range(0, n, frame_stride):
            img = (
                np.asarray(arr[i, ::pixel_stride, ::pixel_stride], dtype=np.float32)
                / 255.0
            )
        ...
        print(
            f"  {campaign}/{dep}: kept {kept}/{len(range(0, n, frame_stride))} "
...
def main(args):
    ...
    print(
        f"Scanning campaigns: {campaigns} "
        f"(frame stride {args.frame_stride}, pixel stride {args.pixel_stride})"
    )
    for campaign in campaigns:
        entries.extend(
            scan_campaign(
                campaign, args.frame_stride, args.pixel_stride, args.mu_weight
            )
        )
