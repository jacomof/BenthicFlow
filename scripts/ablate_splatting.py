...
def eval_row(row, points, rgb, focal, bg, args, device):
    ...
    g = build_surfels(points, rgb, k_overlap=args.k_overlap, thin_ratio=thin,
    ...
    if row["perceptual"] and args.perceptual_steps > 0:
        g, pinfo = refine_surfels_color_perceptual(g, rgb, focal, bg,
                                                   args.rasterize_mode,
                                                   steps=args.perceptual_steps)
        backend = str(pinfo.get("perceptual_backend", "unknown"))
    lift_s = time.time() - t0

    W = rgb.shape[1]
    front, alpha = render_view(g, torch.eye(4, device=device), focal, W, W,
...
def generate_sample(model, rae, campaigns, rng, args, device):
    ...
    rgb, depth = generate_canvas(model, rae, (cond, cond, cond, cond),
                                 n=args.canvas_n, n_steps=args.steps,
                                 stride=args.stride, cfg_scale=args.cfg,
...
def aggregate(records):
    ...
    for row_key, samples in by_row.items():
        ...
        for m in METRIC_ORDER:
            ...
            if vals:
                agg[row_key][m] = dict(mean=float(np.mean(vals)),
    ...
    for row_key, samples in by_row.items():
        ...
        d = {}
        for m in METRIC_ORDER:
            pairs = [(samples[i][m], ref[i][m]) for i in common
            ...
            if pairs:
                ...
                if m in HIGHER_BETTER:
                    ...
                    win = float((diff < 0).mean())
                d[m] = dict(mean=float(diff.mean()), std=float(diff.std()),
                            n=len(diff), win_rate=round(win, 3))
        deltas[row_key] = d
    return agg, deltas
...
def write_tables(agg, deltas, out_dir, n_samples):
    cols = [("psnr", "PSNR $\\uparrow$", 2), ("ssim", "SSIM $\\uparrow$", 4),
            ("lpips", "LPIPS $\\downarrow$", 4), ("l1", "L1 $\\downarrow$", 4)]
    ...
    for row in ROWS:
        ...
        if not a:
            continue
        cells = [CHECK_TEX[row["surfel"]], CHECK_TEX[row["refine"]],
    ...
    # ---- Markdown ----
    md = ["| 2D surfels | Anchored refine | Perceptual | "
          + " | ".join(h.replace("$\\uparrow$", "\u2191").replace("$\\downarrow$", "\u2193")
                       for _, h, _ in cols) + " | n |",
          "|" + "---|" * (3 + len(cols) + 1)]
    for row in ROWS:
        a = agg.get(row["key"])
        if not a:
            continue
        cells = [CHECK_MD[row["surfel"]], CHECK_MD[row["refine"]],
                 CHECK_MD[row["perceptual"]]]
        cells += [fmt_md(a, m, d) for m, _, d in cols]
        cells.append(str(a["n"]))
        md.append("| " + " | ".join(cells) + " |")
    md.append("")
    md.append(f"Paired deltas vs `{REFERENCE_ROW}` (mean of per-sample diffs, "
...
def resolve_ply_sample(spec, records):
    ...
    if str(spec).lower() == "best":
        ...
        print(f"  [ply] 'best' -> sample {best['sample_id']} "
              f"(D_full lpips={best.get('lpips', 'n/a')}, psnr={best['psnr']})")
        return int(best["sample_id"])
    try:
        return int(spec)
    except ValueError:
        print(f"  [ply] WARNING: could not parse --ply-sample '{spec}'; "
              f"skipping export")
        return None
...
"""
    ...
    rgb, depth, camp = generate_sample(model, rae, args.campaigns, rng, args,
                                       device)
    W = depth.shape[1]
    ...
    print(f"  [depth-dbg] depth: shape={tuple(dd.shape)} "
          f"min={float(dd.min()):.5f} max={float(dd.max()):.5f} "
          f"std={float(dd.std()):.5f}")
    if dd.dim() == 3 and dd.shape[0] in (3, 4):
        print(f"  [depth-dbg] !! depth tensor has {dd.shape[0]} leading "
    ...
    if float(dd.std()) < 1e-4:
        print(f"  [depth-dbg] !! depth is CONSTANT (std<1e-4): the flatness is "
    ...
    _z = _pts[:, 2]
    print(f"  [depth-dbg] points: shape={tuple(points.shape)}  Z "
    ...
    if float(_z.std()) < 1e-4 and float(dd.std()) >= 1e-4:
        print(f"  [depth-dbg] !! Z is FLAT after unproject though depth had "
    ...
    relief = {"depth_std": round(d_std, 4),
    ...
    plt.imsave(pdir / "reference_rgb.png",
               np.clip(rgb.detach().cpu().numpy(), 0, 1))
    plt.imsave(pdir / "reference_depth.png", d.numpy(), cmap="viridis")

    print(f"  [ply] sample {sid} ({camp})  depth std={d_std:.4f} "
          f"p5-p95 span={d_span:.4f} (range {z_span_units:.4f})")
    if low_relief:
        print(f"  [ply] WARNING: this sample is nearly flat (depth span "
    ...
    manifest = {"sample_id": sid, "campaign": camp, "relief": relief,
                "low_relief_flag": low_relief, "rows": {}}
    for key in ply_rows:
        metrics, _front, _cov, g = eval_row(row_by_key[key], points, rgb, focal,
                                            bg, args, device)
        # ---- STAGE 3: the surfel means that get written to the .ply ----
        _mz = g["means"].detach().float()[:, 2]
        print(f"  [depth-dbg] {key}: means Z min={float(_mz.min()):.5f} "
              f"max={float(_mz.max()):.5f} std={float(_mz.std()):.5f}")
        if float(_mz.std()) < 1e-4 and float(_z.std()) >= 1e-4:
            print(f"  [depth-dbg] !! {key}: surfel Z flat although the point "
        ...
        print(f"  [ply] wrote {ply_path.name} ({size_mb:.2f} MB)  "
    ...
    print(f"  [ply] open the .ply files in SuperSplat, frame both at the SAME "
...
def save_qualitative(sample_id, rgb, fronts, out_dir):
    ...
    for a in ax:
        a.axis("off")
    fig.tight_layout()
    fig.savefig(qdir / f"sample_{sample_id:04d}.png", dpi=130,
                bbox_inches="tight")
    plt.close(fig)
...
def parse_args():
    ...
    p.add_argument("--canvas-n", type=int, default=1,
    ...
    p.add_argument("--ply-sample", type=str, default="0",
    ...
    p.add_argument("--ply-rows", nargs="+", default=["D_full", "E_iso_full"],
                   choices=[r["key"] for r in ROWS],
    ...
    p.add_argument("--ply-only", action="store_true",
...
def main():
    ...
    (out_dir / "config.json").write_text(json.dumps(
        ...
        | {"rows": [r["key"] for r in rows], "reference_row": REFERENCE_ROW,
           "single_view_only": True}, indent=2))
    ...
    done_ids = {r["sample_id"] for r in records
                if sum(x["sample_id"] == r["sample_id"] for x in records)
                >= len(rows)}
    ...
    if args.ply_only:
        ...
        if ply_sid is None:
            sys.exit("--ply-only needs a concrete --ply-sample: pass an integer "
                     "id, or 'best' with an existing per_sample.jsonl in --out.")
        print(f"[ply-only] exporting example .ply for fig:qualitative-3d "
              f"(sample {ply_sid}, rows {args.ply_rows}) ...")
        export_example_plys(ply_sid, args.ply_rows, model, rae, args, bg,
                            device, out_dir)
    ...
    for sid in range(args.n_samples):
        ...
        rgb, depth, camp = generate_sample(model, rae, args.campaigns, rng,
                                           args, device)
        W = depth.shape[1]
        focal = args.focal or float(W)
        points = unproject(depth, focal, args.z_near, args.z_span)
        ...
        for row in rows:
            metrics, front, cov, _g = eval_row(row, points, rgb, focal, bg,
                                               args, device)
            coverage = cov  # primitive-independent at full coverage; last wins
            rec = dict(sample_id=sid, campaign=camp, row=row["key"],
                       coverage=round(cov, 5), **metrics)
            records.append(rec)
        ...
        cov_flag = "" if coverage is None or coverage > 0.99 else \
            f"  COVERAGE={coverage:.3f}(!)"
        print(f"[{n_done}/{args.n_samples}] sample {sid} ({camp})  "
        ...
        if n_done % 25 == 0 or n_done == args.n_samples:
            ...
            (out_dir / "results.json").write_text(json.dumps(
                dict(n_samples=n_done, single_view_only=True, aggregate=agg,
                     paired_deltas_vs_reference=deltas,
                     reference_row=REFERENCE_ROW), indent=2))
    jf.close()
    ...
    (out_dir / "results.json").write_text(json.dumps(
        dict(n_samples=n_final, single_view_only=True, aggregate=agg,
             paired_deltas_vs_reference=deltas,
             reference_row=REFERENCE_ROW), indent=2))
    ...
    if ply_sid is not None:
        print(f"Exporting example .ply for fig:qualitative-3d "
              f"(sample {ply_sid}, rows {args.ply_rows}) ...")
        export_example_plys(ply_sid, args.ply_rows, model, rae, args, bg,
                            device, out_dir)
