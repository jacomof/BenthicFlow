...
def tensor_to_mesh(
    rgb_tensor, depth_tensor, out_prefix, min_d=3.0, max_d=4.5, focal_mm=14.0
):
...
@torch.no_grad()
def cfm_sample_multidiffusion_hybrid(
    model: UNetCFM,
    cond_left: torch.Tensor | None = None,
    ...
    win_h = torch.sin(torch.linspace(0.05, 0.95, grid, device=device) * torch.pi).view(
        1, 1, grid, 1
    )
    win_w = torch.sin(torch.linspace(0.05, 0.95, grid, device=device) * torch.pi).view(
        1, 1, 1, grid
    )
    win = win_h * win_w
...
def generate_fig2_conditional(rae, cfm, cond, device, out_dir):
    ...
    z_uncond = cfm_sample_cfg(
        cfm,
        cfm.null_cond.unsqueeze(0),
        canvas_shape=(16, 16),
        n_steps=150,
        cfg_scale=1.0,
    )
    rgbd_uncond = rae.decoder(
        cfm.denormalize(z_uncond).permute(0, 2, 3, 1).contiguous()
    ).clamp(0, 1)[0]
    ...
    cond = cond.to(device)
    z_cond = cfm_sample_cfg(
        cfm, cond, canvas_shape=(16, 16), n_steps=150, cfg_scale=3.0
    )
    rgbd_cond = rae.decoder(
        cfm.denormalize(z_cond).permute(0, 2, 3, 1).contiguous()
    ).clamp(0, 1)[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Overlap ratio for MultiDiffusion tiles",
    )
    parser.add_argument(
        "--avg-end-t", type=float, default=1.0, help="Time to switch in MultiDiffusion"
    )
    parser.add_argument(
        "--num-images",
        type=int,
        default=1,
        help="Number of diverse panorama images to generate",
    )
    parser.add_argument(
    ...
    if args.cache_dir:
        try:
            ...
            real_latent = (
                torch.from_numpy(np.array(latents_mm[0]))
                .float()
                .reshape(-1, 768)
                .mean(0, keepdim=True)
            )
