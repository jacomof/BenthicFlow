...
from benthicflow import DEPTH_ROOT, FEAT_ROOT, FIG_ROOT, PROJECT_ROOT, RGB_ROOT
from scripts.extract_features import load_dinov2
from scripts.flux_generation import DEV_ID, REEF_PROMPT, resize_to_multiple_of_16
from scripts.test_generation import DEVICE, TRAIN_SIZE, _DINOv2CLS
...
class CSVSplitDataset(Dataset):
    def __init__(self, split: str, load_rgb: bool = False, campaign: str = None):
            ...
            for row in csv.DictReader(handle):
                ...
                if not all([p.exists() for p in [keys_npy, dino_npy, rgb_npy]]):
                    raise FileNotFoundError(
                        f"Missing keys/DINO/RGB files for {dep_key}: {keys_npy}, {dino_npy}, {rgb_npy}"
                    )
        ...
        print(
            f"Available campaigns are {available_campaigns}, split '{split}' has {len(self.records)} records, load_rgb={self.load_rgb}"
        )
    ...
    def __getitem__(self, i: int):
        ...
        if self.load_rgb:
            rgb_slice = np.array(
                rgb_arr[idx], copy=True
            )  # [S, S, 3] uint8, S = NATIVE_SIZE
...
def build_metrics(feature_extractor, kid_subset_size):
    ...
    if feature_extractor == "dino":
        ...
        kid = KernelInceptionDistance(
            feature=_DINOv2CLS(dino_judge), subset_size=kid_subset_size
        ).to(DEVICE)
        return fid, kid, "DINOv2"
    fid = FrechetInceptionDistance(feature=2048, normalize=True).to(DEVICE)
    kid = KernelInceptionDistance(
        feature=2048, subset_size=kid_subset_size, normalize=True
    ).to(DEVICE)
...
def evaluate_pair(
    ...
    for rgb_real, _ in pbar:
        ...
        if max_per_rank is not None and n_real >= max_per_rank:
            break
    rprint(
        f"[{tag}] real (rank0 shard): {n_real} imgs -- dataloader wait {data_wait:.1f}s, "
    ...
    for rgb_seed, _ in pbar:
        ...
        cos_per_img = F.cosine_similarity(
            seed_pooled, gen_pooled, dim=1
        )  # [B] in [-1, 1]
        ...
        pbar.set_postfix(
            data=f"{gen_data_wait:.1f}s", genupd=f"{gen_time:.1f}s", n_fake=n_fake
        )
        if max_per_rank is not None and n_fake >= max_per_rank:
            break
    rprint(
        f"[{tag}] gen (rank0 shard): {n_fake} imgs -- dataloader wait {gen_data_wait:.1f}s, "
    ...
    if WORLD_SIZE > 1:
        ...
        pm = torch.tensor(
            [pooled_cossim_sum, pooled_cossim_n], device=DEVICE, dtype=torch.float64
        )
    ...
    kid_str = (
        "n/a"
        if out["kid_mean"] is None
        else f"{out['kid_mean']:.5f} +/- {out['kid_std']:.5f}"
    )
    cos_str = (
        "n/a"
        if out["dino_pooled_cossim"] is None
        else f"{out['dino_pooled_cossim']:.5f}"
    )
    rprint(
        f"[{tag}] FID={out['fid']}  KID={kid_str}  DINO_pooled_cossim={cos_str}  "
...
def evaluate_unconditional(pipe, fid, kid, real_loader, args, tag, debug_samples=0):
    ...
    pbar.close()
    rprint(
        f"[{tag}] gen (rank0 shard): {n_fake} imgs -- generate+update {gen_time:.1f}s"
    )
...
def loader_for(split, campaign, args, shuffle=False):
    ...
    if WORLD_SIZE > 1:
        sampler = DistributedSampler(
            ds, num_replicas=WORLD_SIZE, rank=RANK, shuffle=shuffle, drop_last=False
        )
        loader = DataLoader(
            ds,
...
def main(args):
    ...
    if run_uncond:
        ...
        if is_main():
            update_results_json(
                RESULTS_JSON, run_key, config, {"unconditional": section}
            )
    ...
    if run_cond:
        ...
        for campaign in campaigns:
            ...
            per_campaign[campaign] = evaluate_pair(
                ...
                tag=campaign,
                debug_samples=args.debug_samples,
                max_per_rank=cap_per_rank,
            )
        if is_main():
            update_results_json(
                RESULTS_JSON, run_key, config, {"per_campaign": per_campaign}
            )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--steps", type=int, default=28, help="Number of FLUX.2 denoising steps."
    )
    argparser.add_argument(
        "--guidance_scale",
        type=float,
        default=4.0,
        help="Guidance scale (FLUX.2 default 4.0).",
    )
    argparser.add_argument(
        "--n_samples",
        type=int,
