...
def window_starts(length, grid, stride):
    ...
    corrupting the canvas border. Same fix as eval_stitching.py; latent here at
    stride 4/8 (16n - 16 divides evenly) but bites at e.g. stride 6."""
...
@torch.no_grad()
def generate_canvas(model, rae, corners, n=2, n_steps=100, stride=8,
                    cfg_scale=3.0, temperature=1.0, device="cuda", verbose=False,
                    cond_gamma=1.0, pin_corners=False,
                    cond_jitter=0.0, jitter_seed=None):
    ...
    dt = ts[1] - ts[0]
    ys = window_starts(H, grid, stride)
    xs = window_starts(W, grid, stride)
    ...
    jit = None
    if cond_jitter > 0:
        jit = fractal_cond_jitter(len(ys), len(xs), c_tl.shape[-1],
                                  seed=jitter_seed).to(device)
        corner_sep = torch.pdist(torch.cat([c_tl, c_tr, c_bl, c_br], 0)).mean()
        jit *= cond_jitter * corner_sep / (jit.norm(dim=-1).mean() + 1e-8)
        print(f"  cond jitter: mean offset {float(jit.norm(dim=-1).mean()):.2f} = "
...
def make_K(f, w, h, device):
    return torch.tensor([[f, 0, (w - 1) / 2.0],
                         [0, f, (h - 1) / 2.0],
...
def build_surfels(points, colors, k_overlap=0.65, thin_ratio=0.1,
                  max_scale_mult=3.0, normal_smooth=1, opacity=0.999):
    H, W, _ = points.shape
    dev = points.device
    ...
    if normal_smooth > 0:                                # denoise normals (not depth)
        k = 2 * normal_smooth + 1
        n = F.avg_pool2d(n.permute(2, 0, 1)[None], k, 1, normal_smooth)[0].permute(1, 2, 0)
...
# ============================================================================= #
def render_gaussians(g, viewmat, K, width, height, rasterize_mode="classic",
                     render_mode="RGB+ED", near=0.01, far=1e8, clamp_rgb=True):
    ...
    out, alpha, _ = rasterization(
        ...
        near_plane=near, far_plane=far, render_mode=render_mode,
...
def render_view(g, viewmat, focal, ref_size, out_size, ss, bg, mode):
    ...
    comp = F.interpolate(comp.permute(2, 0, 1)[None], size=(out_size, out_size),
...
def refine_surfels_source_view(g, target_rgb, focal, bg, rasterize_mode,
    ...
    for it in range(steps):
        ...
        opacities = torch.sigmoid(_logit(opac0) + opacity_bound * torch.tanh(opacity_delta))
        scales = scale0.clone()
        ...
        gr = dict(means=means, quats=quats, scales=scales,
                  opacities=opacities, colors=colors)
        pred, alpha = render_gaussians(gr, viewmat, K, W, H,
                                       rasterize_mode=rasterize_mode,
                                       clamp_rgb=False)
        ...
        loss = (mse + 0.20 * l1 + 0.05 * grad + 0.02 * coverage
                + 0.002 * color_anchor + 0.05 * opacity_anchor + 0.10 * scale_anchor)
        loss.backward()
        opt.step()
        ...
        if log_every and ((it + 1) % log_every == 0 or it == 0 or it + 1 == steps):
            ...
            print(f"  refine {it + 1:4d}/{steps}: loss={float(loss):.5f} "
...
def _make_perceptual_loss(device):
    ...
    try:
        ...
        print(f"  perceptual: pretrained VGG16 unavailable ({exc.__class__.__name__}: {exc})")
...
def refine_surfels_color_perceptual(g, target_rgb, focal, bg, rasterize_mode,
    ...
    for it in range(steps):
        ...
        if log_every and ((it + 1) % log_every == 0 or it == 0 or it + 1 == steps):
            mse_val = float(mse.detach())
            cur_psnr = 99.0 if mse_val < 1e-12 else -10.0 * np.log10(mse_val)
            print(f"  polish {it + 1:4d}/{steps}: loss={float(loss):.5f} "
                  f"mse={mse_val:.6f} psnr={cur_psnr:.2f} "
...
def ssim(a_hw3, b_hw3, ws=11, sigma=1.5):
    ...
    gk = torch.exp(-(coords ** 2) / (2 * sigma ** 2)); gk /= gk.sum()
...
def write_gaussian_ply(path, g):
    ...
    header = ("ply\nformat binary_little_endian 1.0\n"
              ...
              + "end_header\n").encode()
    rows = np.concatenate([means, np.zeros((n, 3), np.float32), f_dc,
                           op_logit[:, None], log_scale, qt], 1).astype(np.float32)
    with open(path, "wb") as fh:
        fh.write(header); fh.write(rows.tobytes())
...
def save_angle_sheet(g, points, focal, ref_size, out_size, ss, bg, mode, out_path, device):
    tilts, azims = [12, 24], [-25, 0, 25]
    fig, ax = plt.subplots(len(tilts), len(azims), figsize=(4 * len(azims), 4 * len(tilts)))
    for i, tl in enumerate(tilts):
        for j, az in enumerate(azims):
            ...
            ax[i, j].set_title(f"tilt {tl}, azim {az}", fontsize=9); ax[i, j].axis("off")
...
def parse_args():
    ...
    p.add_argument("--normal-smooth", type=int, default=1, help="normal denoise radius (0=off)")
    p.add_argument("--opacity", type=float, default=0.999)
    p.add_argument("--max-scale-mult", type=float, default=3.0)
    p.add_argument("--refine-steps", type=int, default=500,
                   help="anchored appearance-refinement steps; 0 disables")
    p.add_argument("--perceptual-steps", type=int, default=200,
                   help="optional colour-only LPIPS/VGG polish steps after refinement; 0 disables")
    # rendering
    p.add_argument("--ss", type=int, default=2, help="supersample factor for crisp output")
    p.add_argument("--render-size", type=int, default=1200,
                   help="output px of DISPLAY renders, decoupled from the panorama grid; "
                        "raise this (not --ss alone) to sharpen the far field")
    p.add_argument("--rasterize-mode", choices=["classic", "antialiased"], default="antialiased")
...
def main():
    ...
    if args.load_rgbd and args.load_rgbd.exists():
        ...
        rgb, depth = generate_canvas(model, rae, corners, n=args.n, n_steps=args.steps,
    ...
    g = build_surfels(points, rgb, k_overlap=args.k_overlap, thin_ratio=args.thin_ratio,
                      max_scale_mult=args.max_scale_mult, normal_smooth=args.normal_smooth,
    ...
    front_disp, _ = render_view(g, vm_front, focal, Wimg, rs, args.ss, bg, args.rasterize_mode)
    tilt, tilt_a = render_view(g, vm_tilt, focal, Wimg, rs, args.ss, bg, args.rasterize_mode)

    save_panel(rgb, front_disp, tilt, out_dir / "panel.png")
    save_angle_sheet(g, points, focal, Wimg, rs, args.ss, bg, args.rasterize_mode,
                     out_dir / "angles.png", device)
    ...
    if cov_mask.any():
        ...
        masked_psnr = 99.0 if masked_mse < 1e-12 else float(-10 * torch.log10(masked_mse))
    else:
        masked_psnr = 0.0
    metrics = dict(
    ...
    print("Tip: open surfels.ply in SuperSplat (https://playcanvas.com/supersplat) "
