...
def build_test_dataset(test_base: CSVSplitDataset, class_to_idx: dict[str, int],
...
@torch.no_grad()
def generate_conditional(cfm, rae, cond: torch.Tensor, n_steps: int,
...
def main(args):
    ...
    for cls in classes:
        ...
        print(f"  {cls}: {members}")
    print(f"mode={args.mode}  k={args.k}  metric={args.metric}  "
    ...
    if args.mode in ("generated", "both"):
        ...
        cfm.load_state_dict(torch.load(args.cfm_ckpt, map_location=DEVICE, weights_only=True))
        cfm.eval()
    ...
    print(f"train set: {len(train_ds)} images, {n_classes} classes x "
    ...
    if not args.no_figures:
        fig_dir = Path(args.fig_dir) if args.fig_dir else (FIG_ROOT / "cfm" / "knn_campaign")
    ...
    config = {
        ...
        "n_steps": args.n_steps, "cfg_scale": args.cfg_scale, "seed": args.seed,
        "cfm_ckpt": args.cfm_ckpt, "rae_ckpt": args.rae_ckpt,
...
if __name__ == "__main__":
    ...
    p.add_argument("--n_per_class", type=int, default=1500,
    ...
    p.add_argument("--n_steps", type=int, default=50, help="CFM sampling steps (generated mode).")
    p.add_argument("--cfg_scale", type=float, default=3.0, help="CFM guidance scale (generated mode).")
    p.add_argument("--cfm_ckpt", type=str, default=None, help="CFM checkpoint (generated mode).")
    p.add_argument("--rae_ckpt", type=str, default=None, help="RAE checkpoint (generated mode).")
