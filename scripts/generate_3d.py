...
def _load_rgb(path: Path) -> np.ndarray:
	try:
		...
		raise SystemExit("Pillow is required to read RGB images. Install pillow.") from exc
...
def _load_depth(path: Path) -> np.ndarray:
	try:
		...
		raise SystemExit("Pillow is required to read depth images. Install pillow.") from exc
...
def _load_depth_numpy(campaign: str, deployment: str, key: str, label: str) -> np.ndarray:
	...
	print(f"Loaded depth cache for {campaign}/{deployment} with label={label}: {len(keys)} entries.")
...
def _infer_intrinsics(
    width: int, height: int, focal_length_mm: float = 14.0, sensor_width_mm: float = 17.3,
...
def process_depth(depth_m: np.ndarray,
							min_depth=3.0, max_depth=4.5) -> np.ndarray:
	
    depth_norm = (depth_m - np.min(depth_m)) / (np.max(depth_m) - np.min(depth_m) + 1e-8)

    epsilon = 0.5
    linear_depth = 1.0 / (depth_norm + epsilon)

    linear_depth = (linear_depth - np.min(linear_depth)) / (np.max(linear_depth) - np.min(linear_depth))
...
def _normalize_points_unit_cube(points: np.ndarray, centered: bool = True) -> np.ndarray:
	if points.size == 0:
		return points
...
def _points_to_mesh(points: np.ndarray, colors: np.ndarray) -> o3d.geometry.TriangleMesh:
...
def _save_reference_images(
	...
	try:
		...
		raise SystemExit("Pillow is required to save reference images. Install pillow.") from exc
...
def build_argparser() -> argparse.ArgumentParser:
	...
	parser.add_argument("--campaign", type=str, help="Campaign name for cached RGB/depth.")
	parser.add_argument("--deployment", type=str, help="Deployment name for cached RGB/depth.")
...
def main() -> None:
	...
	if use_cache:
		if not args.campaign or not args.deployment:
			raise SystemExit("--campaign and --deployment are required for cached RGB/depth.")
		loader = RGBDImageLoader(args.campaign, 
						   args.deployment, 
						   metric=args.metric,
		...
		if args.key is not None or args.index is not None:
			...
			keys = [args.key if args.key is not None else loader.key_from_index(args.index)]
		else:
	...
	if use_cache:
		...
		for key in keys:
			...
			print(f"Processing key '{key}' with RGB shape {rgb.shape} and depth shape {depth.shape}...")
			if args.fx is None or args.fy is None or args.cx is None or args.cy is None:
				fx, fy, cx, cy = _infer_intrinsics_v2(
