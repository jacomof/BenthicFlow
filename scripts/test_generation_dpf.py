...
from scripts.extract_features_dpf import DirectoryImagesNative, load_dinov2
...
def print_campaign_summary(title, results):
    ...
    rmses = [
        m["depth_rmse"] for m in results.values() if m.get("depth_rmse") is not None
    ]
    sirmses = [
        m["depth_si_rmse"]
        for m in results.values()
...
def evaluate_unconditional(model, rae, metrics, args):
    ...
    with torch.no_grad():
        ...
        for rgb_real, _, _ in pbar:
            ...
            t0 = time.time()
            gen_rgb, _ = generate_batch_unconditionally(
...
def save_rae_reconstruction_debug(
    ...
    for j in range(n):
        ...
        axes[2, j].imshow(
            d_src, cmap="turbo", vmin=float(d_src.min()), vmax=float(d_src.max())
        )
        axes[3, j].imshow(
            d_rec, cmap="turbo", vmin=float(d_src.min()), vmax=float(d_src.max())
        )
        for r in range(4):
            axes[r, j].set_xticks([])
            axes[r, j].set_yticks([])
    ...
    for campaign in campaigns:
        ...
        with torch.no_grad():
            ...
            for rgb_src, depth, dino in tqdm(train_loader, desc=f"{campaign} gen+rae"):
                ...
                gen_rgb, _ = generate_batch_conditionally(
...
def main(args):
    ...
    if args.mode in ("conditional", "both", "all"):
        ...
        cond_res, rae_res = evaluate_conditional_per_campaign(
            model, rae, metrics, args, compute_depth
        )
    ...
    config = {
        ...
        "depth_consistency": compute_depth
        and args.mode in ("conditional", "both", "all"),
    }
