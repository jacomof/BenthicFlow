...
# DPF-Net (BenthicFlow-DPF) variant of scripts/train_cfm_cfg.py.
...
def load_rae_frozen(ckpt_path: Path, device: torch.device) -> RAE:
    rae_cfg = RAEConfig(
        depth_cfg=DepthEncoderConfig(patch_size=14, dim=256, depth=8, num_heads=8, out_dim=256),
        decoder_cfg=ConvDecoderConfig(
            in_dim=1024, hidden_dims=(512, 256, 128), upsample_factors=(2, 7),
...
class CSVSplitDataset(Dataset):
    def __init__(self, split: str, load_rgb: bool = False):
        ...
        with csv_path.open("r", newline="") as handle:
            for row in csv.DictReader(handle):
                campaign, deployment, rel_path = row.get("campaign"), row.get("deployment"), row.get("path")
                if not all([campaign, deployment, rel_path]): continue
                ...
                if filename in depth_key_cache[dep_key]:
                    self.records.append((campaign, deployment, rel_path, depth_key_cache[dep_key][filename]))

        if not self.records:
            raise RuntimeError(
                f"CSVSplitDataset is empty for split '{split}' ({csv_path}). "
                "Check that the key naming (stem vs .jpg) matches the *_keys.npy files."
            )
    ...
    def __getitem__(self, i: int):
        ...
        # depth_arr[idx] is already (1, 256, 256); resize to the 266 native grid
        # so it aligns with RGB/features for grid-snapped cropping (no unsqueeze).
...
@torch.no_grad()
def fit_normalizer(model: UNetCFM, rae: RAE, dataloader: DataLoader, max_batches: int = 100):
    ...
    for i, (rgb, depth, dino) in enumerate(tqdm(dataloader, desc="Fitting Normalizer")):
        ...
        _, fused = rae(dino_spatial.permute(0, 2, 3, 1), depth) # RAE outputs channel-last
    ...
    print(f"Normalizer fitted. Mean range: [{mean.min():.3f}, {mean.max():.3f}], Std range: [{std.min():.3f}, {std.max():.3f}]")


def cfm_loss(model: UNetCFM, z1_norm: torch.Tensor, cond: torch.Tensor, p_uncond: float = 0.1) -> torch.Tensor:
    ...
    if model.training and p_uncond > 0:
        ...
        cond = torch.where(drop_mask.unsqueeze(1), base_model.null_cond.unsqueeze(0), cond)
...
@torch.no_grad()
def evaluate(model: UNetCFM, rae: RAE, dataloader: DataLoader, is_ddp: bool = False,
    ...
    for rgb, depth, dino in dataloader:
        ...
        with amp_autocast(amp):
            _, fused = rae(dino_spatial.permute(0, 2, 3, 1), depth.to(DEVICE, non_blocking=True))
        z1_norm = base_model.normalize(fused.float().permute(0, 3, 1, 2))

        with amp_autocast(amp):
            loss = cfm_loss(base_model, z1_norm, cond, p_uncond=0.0) # No dropout on eval
        bs = rgb.shape[0]
...
@torch.no_grad()
def save_generation_grid(model: UNetCFM, rae: RAE, batch, out_path: Path,
                         n: int = 8, n_steps: int = 50, cfg_scale: float = 3.0):
    ...
    def decode(z_norm):
        ...
        return rgbd[:, :3].clamp(0, 1), rgbd[:, 3:4]   # depth is metric -> no clamp
    ...
    real_depth = depth[:n].to(DEVICE)

    # Depth is metric (Depth-Anything-V2), not [0,1]: colour-map each image with
    # turbo scaled to its own min/max (near=blue, far=red). Clamping to [0,1]
    # like RGB would saturate metric depth to white. Matches train_rae_ddp.py.
    turbo = plt.get_cmap("turbo")
    def depth_to_turbo(depth_t):                          # [n,1,H,W] -> [n,3,H,W] in [0,1]
        out = []
        for d in depth_t:
            dd = d[0]
            vmin, vmax = float(dd.min()), float(dd.max())
            dn = (dd - vmin) / max(vmax - vmin, 1e-6)
            col = torch.from_numpy(turbo(dn.detach().cpu().numpy())[..., :3])
            out.append(col.permute(2, 0, 1).float())
    ...
    rows = [
        ...
        ("Real depth", depth_to_turbo(real_depth)),
        ("Conditional depth", depth_to_turbo(depth_cond)),
        ("Unconditional depth", depth_to_turbo(depth_uncond)),
    ]
...
def main():
    ...
    ap.add_argument("--gen-every", type=int, default=20,
    ...
    train_dl = DataLoader(train_ds, batch_size=args.batch, sampler=train_sampler,
                          shuffle=(train_sampler is None), num_workers=args.workers, drop_last=True,
                          pin_memory=True, persistent_workers=(args.workers > 0))
    val_dl = DataLoader(val_ds, batch_size=args.batch, sampler=val_sampler, shuffle=False,
                        num_workers=2, pin_memory=True, persistent_workers=True)
    test_dl = DataLoader(test_ds, batch_size=args.batch, sampler=test_sampler, shuffle=False,
                         num_workers=2, pin_memory=True, persistent_workers=True)
    ...
    if global_rank == 0:
        ...
        gen_batch = next(iter(DataLoader(gen_ds, batch_size=8, shuffle=False, num_workers=2)))
    ...
    for epoch in range(args.epochs):
        ...
        val_loss = evaluate(model, rae, val_dl, is_ddp=is_ddp, amp=args.amp)   # all-reduce inside == sync point

        if global_rank == 0:
            print(f"Validation Loss: {val_loss:.5f}")
            if (epoch + 1) % args.gen_every == 0 or epoch == args.epochs - 1 or epoch == 0:
                save_generation_grid(model, rae, gen_batch, fig_dir / f"gen_e{epoch:03d}.png")
