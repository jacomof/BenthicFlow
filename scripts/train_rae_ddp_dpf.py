...
class CSVSplitDataset(Dataset):
        ...
        with csv_path.open("r", newline="") as handle:
            ...
            for row in reader:
                ...
                if not depth_npy.exists() or not keys_npy.exists() or not dino_npy.exists():
                    raise FileNotFoundError(f"Missing required files for {dep_key}: {depth_npy}, {keys_npy}, {dino_npy}")
    ...
    def __getitem__(self, i: int):
        ...
        depth = torch.from_numpy(np.array(depth_arr[idx], dtype=np.float32))   # (1, 256, 256)
...
def save_recon_grid(rae: RAE, batch, out_path: Path, n: int = 8):
    ...
    fig, axes = plt.subplots(len(rows), n, figsize=(2.2 * n, 2.4 * len(rows)),
                             squeeze=False)
...
def run_evaluation(rae: RAE, loss_fn: RAELoss, dataloader, 
    ...
    sums = {"l1_rgb": 0.0, "l1_depth": 0.0, "silog_depth": 0.0,
            "depth_grad": 0.0, "lpips": 0.0,
    ...
    for i, (rgb, depth, rgb_latent) in enumerate(dataloader):
        ...
        sums["silog_depth"] += silog_loss(
            depth_p, depth, lam=loss_fn.silog_lambda, eps=loss_fn.silog_eps
        ).item() * b
        sums["depth_grad"] += multiscale_gradient_loss(
            depth_p, depth, num_scales=loss_fn.depth_grad_scales,
...
def build_models() -> tuple[RAE, DinoDiscriminator, RAELoss, TokenLayerNorm]:
    ...
    loss_fn = RAELoss(
        ...
        w_l1_rgb=1.0, w_depth=5.0, w_lpips=1.0, w_gan=0.75,
        depth_loss="silog", silog_lambda=0.85,
        w_depth_grad=0.5, depth_grad_scales=4,
    ).to(DEVICE)
    token_norm = TokenLayerNorm().to(DEVICE)
