...
def parse_args():
    ...
    parser.add_argument('--campaign', type=str, default=None, help='specific campaign to process (overrides split_path)')
...
def test(args):
    ...
    depth_anything.load_state_dict(torch.load(f'{args.depth_anything_folder}/depth_anything_v2_{encoder}.pth', map_location=args.device))
    ...
    dpem = DPEM_model.MainNet(device=args.device, imgSize=256, depth_anything_dir=args.depth_anything_folder).eval()
    ...
    for (campaign, deployment), group in groups:
        ...
        img_list = [generate_src_image_path(campaign, deployment, key) for key in group["key"].tolist()]

        dataset = UIEB_Dataset(img_list, campaign, deployment, depthanything=depth_anything, device=args.device, isTrain=False)
        ...
        print('Test start')
        with torch.no_grad():
            for batch_idx, (data_raw, data_depth, BL, file_name, dst_path, key) in enumerate(dataloader):
                ...
                for i in range(data_raw.shape[0]):
                    ...
                    enc_img = (outputs[i] * 255).to('cpu', dtype=torch.uint8).permute(1, 2, 0)
