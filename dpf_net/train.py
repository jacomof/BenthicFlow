...
def parse_args():
    ...
    parser.add_argument(
        "--lr", type=float, help="learning rate of the models", default=0.001
    )
...
def train(args):
    ...
    dataset = UIEB_Dataset(
        raw_images_path=args.raw_image_path,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data_raw, data_ref, data_depth, BL) in enumerate(dataloader):
            ...
            x_degraded = (
                (outputs * 255.0) * torch.exp(-replicated_x_d * replicated_x_beta_D)
                + replicated_x_B
                * (1 - torch.exp(-replicated_x_beta_B * replicated_x_d))
            ) / 255.0
        ...
        if train_loss < lowest_train_loss:
            ...
            torch.save(
                model.state_dict(),
                os.path.join("./checkpoint", start_time + "-DPF_Net.pth"),
            )
            torch.save(
                dpem.state_dict(),
                os.path.join("./checkpoint", start_time + "-DPEM_finetune.pth"),
            )
