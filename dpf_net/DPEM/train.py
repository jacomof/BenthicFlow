...
def parse_args():
    ...
    parser.add_argument(
        "--lr", type=float, help="learning rate of the models", default=0.0005
    )
...
def train(args):
    ...
    dataset = NYU_Dataset(
        nyu_path=args.onland_image_path,
        water_path=args.underwater_image_path,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (
            ...
            replicated_x_B = (
                x_B.unsqueeze(2)
                .unsqueeze(3)
                .repeat(1, 1, data_onland.shape[2], data_onland.shape[3])
            )
            replicated_x_beta_D = (
            ...
            x_degraded = data_onland * torch.exp(
                -replicated_x_d * replicated_x_beta_D
        ...
        if train_loss < lowest_train_loss:
            ...
            torch.save(
                model.state_dict(),
                os.path.join("./checkpoint", start_time + "-DPEM.pth"),
            )
