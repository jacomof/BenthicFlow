...
class NYU_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_onland = cv2.resize(
            data_onland, (self.size, self.size), interpolation=cv2.INTER_LINEAR
        )

        data_depth_path = self.depth_list[index]
        data_depth = cv2.imread(data_depth_path, cv2.COLOR_BGR2GRAY)
        data_depth = cv2.resize(
            data_depth, (self.size, self.size), interpolation=cv2.INTER_LINEAR
        )
        ...
        abs_depths = (np.max(data_depth) - data_depth + np.min(data_depth)) * (
            max_depth - min_depth
        ) + min_depth

        category = np.random.randint(0, 10)
        beta_D, beta_B = Jerlov(category + 1)
        underwater_img = cv2.imread(
            self.water_list[np.random.randint(0, len(self.water_list))]
        )
        B = pre_B_estimate(underwater_img, self.device, True)
        B = get_B(B[1], beta_D, beta_B)
        degraded_img = data_onland * np.exp(
            -expand_dimension(beta_D, self.size) * np.expand_dims(abs_depths, axis=2)
        ) + expand_dimension(B, self.size) * (
            1
            - np.exp(
                -expand_dimension(beta_B, self.size)
                * np.expand_dims(abs_depths, axis=2)
            )
        )
