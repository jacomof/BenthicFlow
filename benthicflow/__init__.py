...
SCRATCH_ROOT = Path(
    os.environ.get(
        "BENTHICFLOW_SCRATCH_ROOT",
        os.environ.get("REEF_SCRATCH_ROOT", PROJECT_ROOT / "scratch"),
    )
)
SCRATCH_NODE_ROOT = Path(
    os.environ.get(
        "BENTHICFLOW_SCRATCH_NODE_ROOT",
        os.environ.get("REEF_SCRATCH_NODE_ROOT", SCRATCH_ROOT / "node"),
    )
)
...
VARIANT = os.environ.get(
    "BENTHICFLOW_VARIANT", os.environ.get("REEF_VARIANT", "")
).lower()
...
RGB_ROOT = (
    SCRATCH_NODE_ROOT / f"rgb{_SFX}"
)  # precomputed per-deployment uint8 RGB arrays
CKPT_ROOT = SCRATCH_ROOT / f"checkpoints{_SFX}"

# Debug print: This will show up in your SLURM .out file
print(
    f"--- BenthicFlow initialized at {PROJECT_ROOT} (variant: {VARIANT or 'base'}) ---"
)
...
from .rgbd_loader import NATIVE_SIZE, RGBDImageLoader, resize_shorter_then_center_crop
