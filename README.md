...
**Generating Extensible Underwater Environments via Flow Matching** · *ECCV 2026 submission (under review)*
...
(see [§ BenthicFlow-DPF variant](#6--benthicflow-dpf-variant-slurmdpf)).
...
deterministically (fixed seed) into `data_split/` by `slurm/data/split_data.sh`.
...
├── data_split/      train/val/test CSVs (regenerated by slurm/data/split_data.sh)
...
Tested on a SLURM cluster with H100/A100 GPU nodes (training used 4×H100,
evaluation a single A100).
...
**2. Configure `env.sh`** — every job script sources it; edit the scratch
locations for your cluster/user:
...
**4. (Only for 3D lifting)** build the gsplat CUDA extension against this env's
torch, on a GPU node:

```bash
sbatch slurm/setup/install_gsplat.sh
```

### Submitting jobs

**Always submit from the repository root** so `env.sh` resolves and
`PROJECT_ROOT` defaults correctly:

```bash
sbatch slurm/train/train_rae_ddp.sh
```

Job scripts fail fast with a clear error if submitted from anywhere else.
Logs land in `logs/<job-name>_<job-id>.out`.
...
DDP jobs request `--constraint=scratch-node` and copy features/depth/rgb to
node-local NVMe first (with a disk-space guard) before launching `torchrun`.

### 3 · Evaluation (`slurm/eval/`) — paper tables & diagnostics
...
### 6 · BenthicFlow-DPF variant (`slurm/dpf/`)
