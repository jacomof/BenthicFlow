...
class UNetCFM(nn.Module):
    ...
    def __init__(self, latent_dim: int = 1024, cond_dim: int = 768, base_ch: int = 512):
        super().__init__()
        ...
        self.c_embed = nn.Sequential(
            nn.Linear(cond_dim, emb_ch), nn.SiLU(), nn.Linear(emb_ch, emb_ch)
        )
    ...
    def normalize(self, z):
        return (z - self.latent_mean.view(1, -1, 1, 1)) / self.latent_std.view(
            1, -1, 1, 1
        )

    def denormalize(self, z):
        return z * self.latent_std.view(1, -1, 1, 1) + self.latent_mean.view(
            1, -1, 1, 1
        )
