...
def _sincos_2d_pos_embed(grid_h: int, grid_w: int, dim: int) -> torch.Tensor:
    ...
    out_h = torch.cat(
        [torch.sin(gh[:, None] * omega), torch.cos(gh[:, None] * omega)], dim=-1
    )
    out_w = torch.cat(
        [torch.sin(gw[:, None] * omega), torch.cos(gw[:, None] * omega)], dim=-1
    )
...
class DepthEncoder(nn.Module):
    ...
    def __init__(self, cfg: DepthEncoderConfig | None = None):
        ...
        self.patchify = nn.Conv2d(
            1, cfg.dim, kernel_size=cfg.patch_size, stride=cfg.patch_size
        )

        self.blocks = nn.ModuleList(
            [TransformerBlock(cfg.dim, cfg.num_heads) for _ in range(cfg.depth)]
        )
        self.norm_out = TokenLayerNorm()

        self.proj = (
            nn.Linear(cfg.dim, cfg.out_dim) if cfg.out_dim != cfg.dim else nn.Identity()
        )
...
class ConvDecoderConfig:
    ...
    depth_activation: str = field(
        default_factory=lambda: (
            "softplus"
            if (
                os.environ.get("BENTHICFLOW_VARIANT")
                or os.environ.get("REEF_VARIANT", "")
            ).lower()
            == "dpf"
            else "sigmoid"
        )
...
class ConvDecoder(nn.Module):
    ...
    def __init__(self, cfg: ConvDecoderConfig | None = None):
        ...
        self.cfg = cfg
        assert (
            len(cfg.hidden_dims) == len(cfg.upsample_factors) + 1
...
class RAE(nn.Module):
    ...
    def fuse(
        self, rgb_latent: torch.Tensor, depth_latent: torch.Tensor
    ) -> torch.Tensor:
    ...
    def forward(
        self, rgb_latent: torch.Tensor, depth: torch.Tensor
